    ...
    def shiftVoting(self, candidateindex):
        if self.searchDirectory == "/femaleCandidate":
            ...
            prompt = QMessageBox(icon=QMessageBox.Question,
                                 text=f'You have choosen to nominate {self.candidict["maleCHERA"][f"candidate{self.nominatedMale}"]["name"]} and {self.candidict["femaleCHERA"][f"candidate{self.nominatedfemale}"]["name"]}\nContinue?')
            prompt.setStandardButtons(QMessageBox.Yes | QMessageBox.No)
            returned = prompt.exec_()
            if returned == 16384:
                vote(self.candidict, self.nominatedfemale, 'femaleCHERA')
                vote(self.candidict, self.nominatedMale, 'maleCHERA')
            ...
            if event.type() == QEvent.Enter:
                if self.searchDirectory == '/maleCandidate':
                    if object == self.CandiImg1:
                        self.namePlate.setText(self.candidict['maleCHERA']['candidate1']['name'])
                    elif object == self.CandiImg2:
                        self.namePlate.setText(self.candidict['maleCHERA']['candidate2']['name'])
                    elif object == self.CandiImg3:
                        self.namePlate.setText(self.candidict['maleCHERA']['candidate3']['name'])
                    elif object == self.CandiImg4:
                        self.namePlate.setText(self.candidict['maleCHERA']['candidate4']['name'])
                    elif object == self.CandiImg5:
                        self.namePlate.setText(self.candidict['maleCHERA']['candidate5']['name'])
                    elif object == self.CandiImg6:
                        self.namePlate.setText(self.candidict['maleCHERA']['candidate6']['name'])
                elif self.searchDirectory == '/femaleCandidate':
                    if object == self.CandiImg1:
                        self.namePlate.setText(self.candidict['femaleCHERA']['candidate1']['name'])
                    elif object == self.CandiImg2:
                        self.namePlate.setText(self.candidict['femaleCHERA']['candidate2']['name'])
                    elif object == self.CandiImg3:
                        self.namePlate.setText(self.candidict['femaleCHERA']['candidate3']['name'])
                    elif object == self.CandiImg4:
                        self.namePlate.setText(self.candidict['femaleCHERA']['candidate4']['name'])
                    elif object == self.CandiImg5:
                        self.namePlate.setText(self.candidict['femaleCHERA']['candidate5']['name'])
                    elif object == self.CandiImg6:
                        self.namePlate.setText(self.candidict['femaleCHERA']['candidate6']['name'])
