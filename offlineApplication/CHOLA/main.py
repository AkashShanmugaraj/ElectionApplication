    ...
    def shiftVoting(self, candidateindex):
        if self.searchDirectory == "/femaleCandidate":
            ...
            prompt = QMessageBox(icon=QMessageBox.Question,
                                 text=f'You have choosen to nominate {self.candidict["maleCHOLA"][f"candidate{self.nominatedMale}"]["name"]} and {self.candidict["femaleCHOLA"][f"candidate{self.nominatedfemale}"]["name"]}\nContinue?')
            prompt.setStandardButtons(QMessageBox.Yes | QMessageBox.No)
            returned = prompt.exec_()
            if returned == 16384:
                vote(self.candidict, self.nominatedfemale, 'femaleCHOLA')
                vote(self.candidict, self.nominatedMale, 'maleCHOLA')
            ...
            if event.type() == QEvent.Enter:
                if self.searchDirectory == '/maleCandidate':
                    if object == self.CandiImg1:
                        self.namePlate.setText(self.candidict['maleCHOLA']['candidate1']['name'])
                    elif object == self.CandiImg2:
                        self.namePlate.setText(self.candidict['maleCHOLA']['candidate2']['name'])
                    elif object == self.CandiImg3:
                        self.namePlate.setText(self.candidict['maleCHOLA']['candidate3']['name'])
                    elif object == self.CandiImg4:
                        self.namePlate.setText(self.candidict['maleCHOLA']['candidate4']['name'])
                    elif object == self.CandiImg5:
                        self.namePlate.setText(self.candidict['maleCHOLA']['candidate5']['name'])
                    elif object == self.CandiImg6:
                        self.namePlate.setText(self.candidict['maleCHOLA']['candidate6']['name'])
                elif self.searchDirectory == '/femaleCandidate':
                    if object == self.CandiImg1:
                        self.namePlate.setText(self.candidict['femaleCHOLA']['candidate1']['name'])
                    elif object == self.CandiImg2:
                        self.namePlate.setText(self.candidict['femaleCHOLA']['candidate2']['name'])
                    elif object == self.CandiImg3:
                        self.namePlate.setText(self.candidict['femaleCHOLA']['candidate3']['name'])
                    elif object == self.CandiImg4:
                        self.namePlate.setText(self.candidict['femaleCHOLA']['candidate4']['name'])
                    elif object == self.CandiImg5:
                        self.namePlate.setText(self.candidict['femaleCHOLA']['candidate5']['name'])
                    elif object == self.CandiImg6:
                        self.namePlate.setText(self.candidict['femaleCHOLA']['candidate6']['name'])
