...
class MainScreenSwachBarathCoord(QWidget):
    f = open('../data.json')
    ...
    def shiftVoting(self, candidateindex):
        if self.searchDirectory == "/personCandidate":
            self.nominatedPerson = candidateindex
            prompt = QMessageBox(icon=QMessageBox.Question,text=f'You have choosen to nominate {self.candidict["SwachBarathCoord"][f"candidate{self.nominatedPerson}"]["name"]}\nContinue?')
            prompt.setStandardButtons(QMessageBox.Yes | QMessageBox.No)
            returned = prompt.exec_()
            if returned == 16384:
                vote(self.candidict, self.nominatedPerson, 'SwachBarathCoord')
            ...
            if event.type() == QEvent.Enter:
                if self.searchDirectory == '/personCandidate':
                    if object == self.CandiImg1:
                        self.namePlate.setText(self.candidict['SwachBarathCoord']['candidate1']['name'])
                    elif object == self.CandiImg2:
                        self.namePlate.setText(self.candidict['SwachBarathCoord']['candidate2']['name'])
                    elif object == self.CandiImg3:
                        self.namePlate.setText(self.candidict['SwachBarathCoord']['candidate3']['name'])
                    elif object == self.CandiImg4:
                        self.namePlate.setText(self.candidict['SwachBarathCoord']['candidate4']['name'])
                    elif object == self.CandiImg5:
                        self.namePlate.setText(self.candidict['SwachBarathCoord']['candidate5']['name'])
                    elif object == self.CandiImg6:
                        self.namePlate.setText(self.candidict['SwachBarathCoord']['candidate6']['name'])
