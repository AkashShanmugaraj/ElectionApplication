    ...
    def shiftVoting(self, candidateindex):
        if self.searchDirectory == "/femaleCandidate":
            ...
            prompt = QMessageBox(icon=QMessageBox.Question,
                                 text=f'You have choosen to nominate {self.candidict["maleCulturalSEC"][f"candidate{self.nominatedMale}"]["name"]} and {self.candidict["femaleCulturalSEC"][f"candidate{self.nominatedfemale}"]["name"]}\nContinue?')
            prompt.setStandardButtons(QMessageBox.Yes | QMessageBox.No)
            returned = prompt.exec_()
            if returned == 16384:
                vote(self.candidict, self.nominatedfemale, 'femaleCulturalSEC')
                vote(self.candidict, self.nominatedMale, 'maleCulturalSEC')
            ...
            if event.type() == QEvent.Enter:
                if self.searchDirectory == '/maleCandidate':
                    if object == self.CandiImg1:
                        ...
                        self.namePlate.setText(self.candidict['maleCulturalSEC']['candidate2']['name'])
                    elif object == self.CandiImg3:
                        self.namePlate.setText(self.candidict['maleCulturalSEC']['candidate3']['name'])
                    elif object == self.CandiImg4:
                        self.namePlate.setText(self.candidict['maleCulturalSEC']['candidate4']['name'])
                    elif object == self.CandiImg5:
                        self.namePlate.setText(self.candidict['maleCulturalSEC']['candidate5']['name'])
                    elif object == self.CandiImg6:
                        self.namePlate.setText(self.candidict['maleCulturalSEC']['candidate6']['name'])
                elif self.searchDirectory == '/femaleCandidate':
                    if object == self.CandiImg1:
                        self.namePlate.setText(self.candidict['femaleCulturalSEC']['candidate1']['name'])
                    elif object == self.CandiImg2:
                        self.namePlate.setText(self.candidict['femaleCulturalSEC']['candidate2']['name'])
                    elif object == self.CandiImg3:
                        self.namePlate.setText(self.candidict['femaleCulturalSEC']['candidate3']['name'])
                    elif object == self.CandiImg4:
                        self.namePlate.setText(self.candidict['femaleCulturalSEC']['candidate4']['name'])
                    elif object == self.CandiImg5:
                        self.namePlate.setText(self.candidict['femaleCulturalSEC']['candidate5']['name'])
                    elif object == self.CandiImg6:
                        self.namePlate.setText(self.candidict['femaleCulturalSEC']['candidate6']['name'])
